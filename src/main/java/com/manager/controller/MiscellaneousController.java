...
import com.manager.model.EmailSubscribeRequestWrapper;
...
public class MiscellaneousController {
	...
	public ResponseEntity<ContactMeResponseWrapper> contactMe(
		...
		logger.info("Started the execution for the contact me request with masterTxnRefNo :: " + masterTxnRefNo);
		...
		logger.info("Finished the execution for the contact me request with masterTxnRefNo :: " + masterTxnRefNo);
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "Contact me API is reachable"),
			...
			@ApiResponse(code = 500, message = "Internal Server Error"),
			@ApiResponse(code = 404, message = "Contact me API is not reachable") })
	@ApiOperation(value = "email subscribe ", notes = "email subscribe")
	@PostMapping(value = "/emailSubscribe", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<EmailSubscribeResponseWrapper> emailSubscribe(
			@RequestBody EmailSubscribeRequestWrapper emailSubscribeRequestWrapper,
			@RequestHeader("masterTxnRefNo") String masterTxnRefNo, @RequestHeader("channel") String channel) {
		logger.info("Started the execution for the email subscribe request with masterTxnRefNo :: " + masterTxnRefNo);
		HttpStatus httpStatus = null;
		httpStatus = HttpStatus.OK;
		ResponseHeader responseHeader = new ResponseHeader();
		...
		emailSubscribeResponse = miscellaneousService.emailSubscribeService(emailSubscribeRequestWrapper.getEmailSubscribe(),
				masterTxnRefNo, channel);
		if (emailSubscribeResponse.getSubscribeStatus().equals("0")) {
			CommonUtils.generateHeaderForSuccess(responseHeader);
		} else {
			CommonUtils.generateHeaderForGenericError(responseHeader);
		}
		...
		logger.info("Finished the execution for the email subscribe request with masterTxnRefNo :: " + masterTxnRefNo);
		return new ResponseEntity<>(emailSubscribeResponseWrapper, httpStatus);
